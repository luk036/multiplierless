...
class TestCreateLowpassCaseParams:
    def test_returns_oracle_with_correct_interface(self):
    ...
    def test_assess_feas_returns_cut_for_violation(self):
    ...
    def test_assess_feas_large_vector_violates(self):
        oracle = create_lowpass_case_params(16, 0.12, 0.20, 0.125, 0.125, 15)
        x = np.ones(16) * 10.0
        result = oracle.assess_feas(x)
        assert result is not None
    ...
    def test_assess_optim_returns_cut_and_value(self):
        oracle = create_lowpass_case_params(16, 0.12, 0.20, 0.125, 0.125, 15)
        x = np.zeros(16)
        gamma = oracle.sp_sq
    ...
    def test_assess_feas_negative_x0_triggers_gradient(self):
        oracle = create_lowpass_case_params(16, 0.12, 0.20, 0.125, 0.125, 15)
        x = np.zeros(16)
        x[0] = -1.0
        result = oracle.assess_feas(x)
    ...
    def test_assess_feas_with_different_parameters(self):
...
class TestMain:
    def test_main_no_args_returns_one(self):
        ret = main([])
        assert ret == 1
    ...
    def test_main_default_argv_empty(self, monkeypatch):
        import sys
    ...
    def test_main_optimization_failure(self, tmp_path):
        import json
    ...
    def test_main_with_valid_spec(self, tmp_path):
        spec = {
            "filter_order": 32,
            "passband_edge": 0.12,
            "stopband_edge": 0.20,
    ...
    def test_main_with_root_spectral_method(self, tmp_path):
        spec = {
            "filter_order": 32,
            "passband_edge": 0.12,
            "stopband_edge": 0.20,
    ...
    def test_main_with_verilog_output(self, tmp_path):
        spec = {
            "filter_order": 32,
            "passband_edge": 0.12,
            "stopband_edge": 0.20,
